...
| Dependabot | **Batch triage (2026-04-21):** PRs **#6, #9, #10** **squash-merged** after fresh CI/CodeQL. **#2–#5** (conflicting `go.sum`) addressed by a **maintainer commit** on `main` (`go get` / `go mod tidy` for rekor, sigstore stack, grpc, go-tuf; **sigstore resolves to v1.10.5** with **timestamp-authority v2.0.6**). **#7** (`actions/setup-go` v6) and **#8** (`github/codeql-action` v4): branches were **behind** `main` until **`@dependabot rebase`** comments (2026-04-22); fresh CI/CodeQL **green** on PR heads; **`gh pr merge --squash`** then **succeeded** (merge commits on **`main`**). **Note:** this **`gh`** OAuth token (**`repo`**, no **`workflow`**) still **cannot** `git push` **local** edits to `.github/workflows/*` (**GH013**) or **`gh pr update-branch`** on workflow PRs; prefer **Dependabot rebase** + **merge on GitHub** for workflow bumps when local push is blocked. **`main`** also has **README CI/CodeQL badges** and repo **homepage/topics** polish (2026-04-22). Re-check **Dependabot alerts** after workflow bumps. |
