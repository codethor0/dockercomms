...
| Dependabot | **Batch triage (2026-04-21):** PRs **#6, #9, #10** **squash-merged** after fresh CI/CodeQL. **#2–#5** (conflicting `go.sum`) addressed by a **maintainer commit** on `main` (`go get` / `go mod tidy` for rekor, sigstore stack, grpc, go-tuf; **sigstore resolves to v1.10.5** with **timestamp-authority v2.0.6**). **#7** (`actions/setup-go` v6) and **#8** (`github/codeql-action` v4) **remain open** on GitHub. For the active **`gh`** OAuth token (**`repo`** without **`workflow`**), verified failures include: **`git push`** of workflow files (**GH013**), **`gh pr update-branch`**, and **`gh pr merge`** (**including `--admin`**) with *refusing ... without `workflow` scope*. **`gh auth refresh -s workflow`** requires **interactive** device/browser completion. **2026-04-22:** `main` includes **README CI/CodeQL badges**; workflow action versions on **`main`** stay at **#7**/**#8** pre-merge versions until a **browser session** merge or a **`workflow`-scoped** token is used. Re-check **Dependabot alerts** after those PRs land. |
